...
const CACHE = 'yp-hymnbook-v6';
const ASSETS = ['./', './index.html', './editor.html', './apple-touch-icon-v2.png'];
...
self.addEventListener('fetch', (e) => {
  ...
  // Only pages ignore the query string. Assets match exactly, so a versioned
  // filename or ?v= actually reaches the network instead of resolving to the
  // stale copy saved under the bare name.
  const lookup = caches.match(request, { ignoreSearch: isPage(request) }).then((cached) => {
